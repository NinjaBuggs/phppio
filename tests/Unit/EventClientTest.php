...
class EventClientTest extends TestCase
{
    ...
    {
        $this->eventClient->setUser(1, array('age' => 20), new \DateTime());
        ...
        $this->assertEquals(
            'http://localhost:7070/events.json?accessKey=j4jIdbq59JsF2f4CXwwkIiVHNFnyNvWXqMqXxcIbQDqFRz5K0fe9e3QfqjKwvW3O',
            $request->getUri()
        );
    }
    ...
    {
        $this->eventClient->unsetUser(1, array('age' => 20), new \DateTime());
        $result = array_shift($this->container);
        /** @var Request $request */
        $request = $result['request'];
        $body = json_decode($request->getBody(), true);
    ...
    /**
     * @expectedException \phppio\PredictionIOException
     */
    public function testUnsetUserWithoutProperties()
    {
        $this->eventClient->unsetUser(1, array(), new \DateTime());
    }

    public function testDeleteUser()
    {
        $this->eventClient->deleteUser(1, new \DateTime());
        $result = array_shift($this->container);
        /** @var Request $request */
        $request = $result['request'];
        $body = json_decode($request->getBody(), true);
    ...
    {
        $this->eventClient->setItem(1, array('type' => 'book'), new \DateTime());
        $result = array_shift($this->container);
        /** @var Request $request */
        $request = $result['request'];
        $body = json_decode($request->getBody(), true);
    ...
    {
        $this->eventClient->unsetItem(1, array('type' => 'book'), new \DateTime());
        $result = array_shift($this->container);
        /** @var Request $request */
        $request = $result['request'];
        $body = json_decode($request->getBody(), true);
    ...
    {
        $this->eventClient->unsetItem(1, array(), new \DateTime());
    }

    public function testDeleteItem()
    {
        $this->eventClient->deleteItem(1, new \DateTime());
        $result = array_shift($this->container);
        /** @var Request $request */
        $request = $result['request'];
        $body = json_decode($request->getBody(), true);
    ...
    {
        $this->eventClient->recordUserActionOnItem('view', 1, 888, array('count' => 2), new \DateTime());
        $result = array_shift($this->container);
        /** @var Request $request */
        $request = $result['request'];
        $body = json_decode($request->getBody(), true);
